...
import { Edit2, Trash2, GraduationCap, Search, Loader2 } from 'lucide-react'
import { studentService, userService } from '../../services/eventService'
...
export default function Students() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editItem.studentId) {
        await studentService.update(editItem.studentId, {
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try {
      if (deleteItem?.studentId) {
        await studentService.remove(deleteItem.studentId)
      }
  ...
  const merged = users
    ...
    .map(u => {
      const student = students.find(s => s.userId === u.userId)
      return {
        userId: u.userId,
  ...
  const filtered = merged.filter(s =>
    !search || s.enrollmentNo?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Students" subtitle={`${merged.length} registered student accounts`} />
            ...
            <tbody>
              {filtered.map(s => (
                <tr key={s.userId}>
                  ...
                  <td><span className="font-mono text-xs text-brand-300">{s.enrollmentNo || '—'}</span></td>
                  <td>{s.course || '—'}</td>
                  <td>{s.year ? <Badge variant="info">Year {s.year}</Badge> : '—'}</td>
                  <td>{s.phone || '—'}</td>
                  <td>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Email" required>
              <input type="email" className="input-dark" placeholder="student@college.edu" value={form.email} onChange={set('email')} />
            </FormField>
          </div>
          <FormField label="Enrollment No">
            <input className="input-dark" placeholder="e.g. CE2021001" value={form.enrollmentNo} onChange={set('enrollmentNo')} />
          </FormField>
          <FormField label="Course">
            <input className="input-dark" placeholder="e.g. B.Tech CSE" value={form.course} onChange={set('course')} />
          </FormField>
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Phone">
              <input className="input-dark" placeholder="+91 9876543210" value={form.phone} onChange={set('phone')} />
            </FormField>
          </div>
          <div className="grid grid-cols-2 gap-3">
      ...
      <ConfirmModal open={!!deleteItem} onClose={() => setDeleteItem(null)} onConfirm={handleDelete}
        loading={deleting} title="Remove Student"
        message={`Delete student "${deleteItem?.name || deleteItem?.enrollmentNo}" account?`} />
    </div>
  )
}
